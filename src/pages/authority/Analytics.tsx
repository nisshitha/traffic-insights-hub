...
import { Loader2, TrendingUp, Clock, Gauge, Target } from 'lucide-react';
...
const AuthorityAnalytics = () => {
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      ...
      const { data } = await supabase
        ...
        .select('hour, avg_speed, congestion_frequency')
  ...
  const avgSpeed = analyticsData.length > 0 
    ? (analyticsData.reduce((sum, d) => sum + (d.avg_speed || 0), 0) / analyticsData.length).toFixed(1)
    : '0';
  
  const peakHours = analyticsData
    .filter(d => d.congestion_frequency > 70)
    .map(d => `${d.hour}:00`)
    .slice(0, 3);

  const avgCongestion = analyticsData.length > 0
    ? (analyticsData.reduce((sum, d) => sum + (d.congestion_frequency || 0), 0) / analyticsData.length).toFixed(0)
    : '0';

  const chartData = analyticsData.map(d => ({
    time: `${d.hour}:00`,
    speed: d.avg_speed,
    congestion: d.congestion_frequency
  }));
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="font-display text-3xl font-bold mb-2">Route Analytics</h1>
            <p className="text-muted-foreground">Performance analysis for major Chennai routes</p>
          </div>
                    ...
                    <tbody>
                      {chartData.map((row, i) => (
                        <tr key={i} className="border-b last:border-0">
                          ...
                          <td className="py-3 px-4">{row.speed?.toFixed(1)} km/h</td>
                          <td className="py-3 px-4">{row.congestion}%</td>
                          <td className="py-3 px-4">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                              row.congestion > 70 ? 'bg-red-100 text-red-700' :
                              row.congestion > 40 ? 'bg-amber-100 text-amber-700' :
